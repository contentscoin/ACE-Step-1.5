...
| **S5 — 완료** | `api/gateway/composition.ts`의 `composeGateway(config)` — `gateway-harness.ts`와 **같은 순서로** 페이크를 실물로 치환: pg `Pool`(계정·자산), Redis(세션·로그인 시도), `AceEngineAdapter` + HTTP 전송, DSP HTTP 클라이언트, 파일시스템 스토어, `HealthMonitor`, 레지스트리 → `EngineLicensePort` 어댑터(`engine-catalogue.ts`, **33.2 판정**을 기록하고 디스크립터의 주장은 기록하지 않음). `server.ts`가 그것을 부르고 `main.ts`가 `npm start`. 설정은 스토어 URL 둘 + 시크릿만 필수, 엔진(8001)·사이드카(8002)·`data/objects`는 기본값. `GET /ready`가 DB·Redis·DSP·엔진 가용성을 보고. `npm run db:migrate` + `MUSICSTUDIO_MIGRATE_ON_START`. **발견한 구멍**: `schedulePolling`이 프로덕션 코드 어디에서도 호출되지 않았음 — 제출된 잡은 영원히 `pending`. `SelfPollingJobOrchestrator`(수락 시 폴 루프 무장, 취소 시 해제, 종료 시 정리)와 `startTimeoutSweep`(5.8, 60초)이 이를 닫음. **v0로 이름 붙인 것**: 잡 스토어·큐·이벤트 버스 인메모리(재시작 시 진행 중 잡 소실; BullMQ는 포트 구현 셋), 크레딧 `free*` 쌍, 가격 permissive, 모더레이션 미합성 — 전부 이 파일의 이름 있는 값. `bullmq`는 **추가하지 않음**: 단일 프로세스가 폴링까지 하므로 v0에 워커 프로세스가 필요 없음 | `composition-root.test.ts`가 **`npm start`가 부르는 바로 그 함수**를 실제 PostgreSQL·Redis·**실제 DSP 사이드카**(CI `database` 잡이 사이드카를 기동)에 대해 조립, 엔진만 스크립트: `/auth/register` → 로그인(실제 bcrypt) → `POST /v1/songs/simple` 202 → 아무도 `pollOnce`를 부르지 않았는데 폴 → 성공 → `audio_asset` 행(owner·engine·`object_key`·48 kHz·provenance `watermarkId`/`MIT`/commercial) → 파일시스템 객체가 `audio/flac`이고 첫 4바이트 `fLaC`. 기동 시 엔진을 **20.8의 임계인 세 번** 프로브 — 레지스트리는 엔진을 `available`로 등록하고 연속 실패 3회 전에는 그대로 두므로, 두 번이면 죽은 엔진이 첫 1분간 라우팅되고 `/ready`도 그렇게 말했을 것(실제로 그랬음; 죽은 엔진 케이스가 잡음). 로컬 `npm start` 실기동(ACE-Step 미기동): `/health` ok, `/ready` 200 `{database:ok, redis:ok, dsp:ok, engine:unavailable, engineLastCheck:failure}` → `degraded`, `/auth/register` 201, SIGTERM 정상 종료. `test:db` 7파일 106건(사이드카 있음/없음 양쪽), 샤드 4개 3,335건, 단위 21건(폴 루프·스윕) |
