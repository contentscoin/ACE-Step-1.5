...
export function Player({ asset }: PlayerProps): ReactNode {
  ...
  // The playhead. `elapsedMs` is wall time since the transport started; where that *lands* in the
  // asset is the domain's answer, which is what makes a loop's seam right.
  useEffect(() => {
    if (!playing) return;
